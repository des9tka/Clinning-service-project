...
const EmployeesBuilder = ({status, order_id, employee}) => {
    ...
    return (
        <div className={'order-employee-wrap'}>
            ...
            {employee?.profile.user_photo && <img onClick={() =>  window.open(`${BASE_URL + employee.profile?.user_photo}`, '_blank')} className={'order-employee-img'} src={`${BASE_URL + employee.profile?.user_photo}`} alt="user_photo"/>}
