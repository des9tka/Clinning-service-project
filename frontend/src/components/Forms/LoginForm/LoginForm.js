...
const LoginForm = () => {
    ...
    return (
        <form className={'login-form'} onSubmit={handleSubmit(log)}>
            {state.message !== 'Not active.' && <div className={'login-error-div'}>{state.message}</div>}
            {state.message === 'Not active.' && <div>Your account is not active. <br/> Send me <a className={'activate-request-link'} onClick={() => activate()}>activation link</a>.</div>}
